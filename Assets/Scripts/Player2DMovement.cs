...
{   // criando variáveis privadas editáveis no inspector
    ...
    private float playerCurrentSpeed; //velocidade atual do player
    private float playerTargetSpeed; // velocidade-alvo do player, que é determinada por quanto o usuário pressiona o direcional horizontal
    ...
    private void Update()
    {
        ...
        else
        {
            if(doubleJumpAvailable == true)
            {
                ...
                rbPlayer.AddRelativeForce(0f, JumpHeight * jumpInput * 10000f, 0f);
                if(jumpInput == 1f)
                {
    ...
    private void FixedUpdate()
    {
        ...
        playerCurrentSpeed = Mathf.Lerp(playerCurrentSpeed, playerTargetSpeed, playerAcceleration * Time.deltaTime);
        ...
        if (hInput == 0.0f)
        {
            if (Mathf.Abs(playerCurrentSpeed) < 0.01f)
            {
        ...
        {
            if (Mathf.Abs(playerCurrentSpeed) > (playerTopSpeed * 0.01f) * 0.95f)
            {
                playerCurrentSpeed = (playerCurrentSpeed / Mathf.Abs(playerCurrentSpeed)) * (playerTopSpeed * 0.01f);
            }
        }

        // cria o vector 3 requerido pelo translate abaixo
        Vector3 playerTranslate = new Vector3(playerCurrentSpeed, 0f, 0f);
